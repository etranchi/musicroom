...
module.exports = function (io) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('updateStatus', async (roomID, trackID) => {
            try {
                ...
                /* =============== */
                let event = await ftSocket.updateStatus(roomID, trackID)
                
                io.sockets.in(roomID).emit('updateStatus', event.playlist.tracks.data, trackID);
            }catch (e) {
                io.sockets.in(roomID).emit('error', e.message);
            }
        })
